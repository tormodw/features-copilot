...
## Overview
This repository contains a complete C++ home automation system designed for energy cost optimization based on multiple sensor inputs.
...
2. **Header-Only Implementation**: Most classes are header-only for simplicity and ease of integration
3. **Smart Pointers**: Uses shared_ptr for memory safety and proper resource management
4. **Threshold-Based Logic**: Implements hysteresis to prevent rapid state changes
5. **Simulation Mode**: Provides mock MQTT/HTTP for testing without external dependencies
